...
class FedSystem(object):
    ...
    def server_excute(self):
        ...
        for r in range(start, args.n_round):
            ...
            with torch.no_grad():
                ...
                if args.pruing_p > 0:
                    self.pruning_server(args.pruing_p, new_param, new_omega)
                for name, param in self.server_model.named_parameters():
    ...
    def client_update(self, client_idx, round_num, lr):
        ...
        for name, param in self.client_model_set[client_idx].named_parameters():
            new_omega[name] = 1 / (1 - data_alpha) * (self.server_omega[name] - data_alpha * self.client_omega_set[client_idx][name])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logname', type=str, default='Curv_Cifar10')
