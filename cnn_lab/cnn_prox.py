...
class FedSystem(object):
    ...
    def __init__(self, args, filename_suffix):
        ...
        wandb.init(config=config, project=args.projectname, reinit=True, resume='allow', id=filename_suffix.split('_t')[0])
    ...
    def get_csd_loss(self, client_idx, mu):
        loss_set = []
    ...
    def client_update(self, client_idx, round_num, lr):
        ...
        for epoch in range(args.n_epoch):
            for batch_idx, (data, target) in enumerate(self.train_loader_group[client_idx]):
                ...
                csd_loss = self.get_csd_loss(client_idx, new_mu) if args.csd_importance > 0 else 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logname', type=str, default='Prox_Cifar10')
    parser.add_argument('--method', type=str, default='Prox')
